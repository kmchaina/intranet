...
@section('page-subtitle', 'Schedule a new event for NIMR staff')
...
@section('content')
                ...
                <form method="POST" action="{{ route('events.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100"
                            placeholder="Provide details about the event">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="location" name="location" value="{{ old('location') }}"
                                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100"
                                placeholder="e.g., NIMR Headquarters">
                            @error('location')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <textarea id="venue_details" name="venue_details" rows="2"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100"
                            placeholder="Additional venue information, directions, equipment available, etc.">{{ old('venue_details') }}</textarea>
                        @error('venue_details')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">RSVP Settings</h3>

                        <!-- Requires RSVP -->
                        <div class="flex items-center mb-4">
                            <input type="checkbox" id="requires_rsvp" name="requires_rsvp" value="1"
                                {{ old('requires_rsvp') ? 'checked' : '' }}
                                class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                                onchange="toggleRsvpFields()">
                            <label for="requires_rsvp" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <!-- Visibility Settings -->
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Event Visibility</h3>

                        <div>
                            <label for="visibility_scope"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Who can see this event? *
                            </label>
                            <select id="visibility_scope" name="visibility_scope" required
                                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100"
                                onchange="toggleTargetingFields()">
                                @foreach ($visibilityScopes as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('visibility_scope', 'all') === $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div id="targeting-fields" class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6"
                            ...
                            <!-- Target Centres -->
                            <div>
                                <label for="target_centres"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Target Centres
                                </label>
                                <select id="target_centres" name="target_centres[]" multiple
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100">
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Hold Ctrl/Cmd to select multiple
                                </p>
                            </div>

                            <!-- Target Stations -->
                            <div>
                                <label for="target_stations"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Target Stations
                                </label>
                                <select id="target_stations" name="target_stations[]" multiple
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100">
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <button type="submit"
                            class="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                            Create Event
                        </button>
                    </div>
                </form>
    ...
    <script>
        ...
        function toggleTargetingFields() {
            const select = document.getElementById('visibility_scope');
