@extends('layouts.dashboard') @section('title', 'Announcements') @section('page-title', 'Announcements') @section('page-subtitle',
'Stay updated with the latest news from NIMR') @section('content') <div class="p-6"> <!-- Header Section -->
        <div class="bg-white/95 backdrop-blur-md rounded-xl border border-gray-200 shadow-lg p-6 mb-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                ...
                @if (auth()->user()->canCreateAnnouncements())
                    <div class="mt-4 lg:mt-0"> <a href="{{ route('announcements.create') }}"
                            ...
                            </svg> Create Announcement </a> </div>
                    @endif
            </div>
        </div> <!-- Search and Filter Section -->
        <div class="bg-white/95 backdrop-blur-md rounded-xl border border-gray-200 shadow-lg p-6 mb-6">
            <div class="flex flex-col lg:flex-row lg:items-center gap-4"> <!-- Search Bar -->
                <div class="flex-1">
                    <div class="relative"> <svg
                            class="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg> <input type="text" id="searchInput" placeholder="Search announcements..."
                ...
                <div class="lg:w-48"> <select id="categoryFilter"
                ...
                <div class="lg:w-48"> <select id="priorityFilter"
                        class="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-800 shadow-sm">
            ...
            <div id="announcementsList" class="divide-y divide-gray-100">
                @forelse($announcements as $announcement)
                    <div class="announcement-item p-6 hover:bg-gray-50 transition-colors duration-150"
                        ...
                        <div class="flex items-start space-x-4"> <!-- Priority & Read Status Indicator -->
                            <div class="flex-shrink-0 flex items-center space-x-2"> <!-- Priority Icon -->
                                <div
                                    class="w-10 h-10 rounded-lg flex items-center justify-center @if ($announcement->priority === 'high') bg-red-100 text-red-600 @elseif($announcement->priority === 'medium') bg-yellow-100 text-yellow-600 @else bg-green-100 text-green-600 @endif">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        @else
                                            ...
                                            @endif
                                    </svg> </div> <!-- Unread Indicator -->
                                @if (!$announcement->isReadBy(auth()->user()))
                                    <div class="w-3 h-3 bg-blue-500 rounded-full"></div>
                                    @endif
                            </div> <!-- Content Area -->
                            <div class="flex-1 min-w-0"> <!-- Header Row -->
                                    ...
                                    <div class="flex-1">
                                        <h3
                                            ...
                                            <a href="{{ route('announcements.show', $announcement) }}" class="block">
                                                {{ $announcement->title }} </a> </h3> <!-- Meta Info -->
                                        <div class="flex items-center space-x-4 text-sm text-gray-500 mt-1">
                                            <span>{{ $announcement->published_at->format('M j, Y') }}</span> <span>•</span>
                                            <span
                                                class="capitalize">{{ str_replace('_', ' ', $announcement->target_scope) }}</span>
                                            @if ($announcement->attachments->count() > 0)
                                                <span>•</span> <span class="flex items-center"> <svg class="w-4 h-4 mr-1"
                                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                                            d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                                                    </svg> {{ $announcement->attachments->count() }} file(s) </span>
                                                @endif
                                        </div>
                                    </div> <!-- Category Badge -->
                                    <div class="flex-shrink-0 ml-4"> <span
                                            class="inline-flex px-3 py-1 text-xs font-medium rounded-full @if ($announcement->category === 'urgent') bg-red-100 text-red-800 @elseif($announcement->category === 'event') bg-blue-100 text-blue-800 @elseif($announcement->category === 'policy') bg-purple-100 text-purple-800 @elseif($announcement->category === 'training') bg-indigo-100 text-indigo-800 @else bg-gray-100 text-gray-800 @endif">
                                            {{ ucfirst($announcement->category) }} </span> </div>
                                </div> <!-- Content Preview -->
                                <p class="text-gray-600 text-sm line-clamp-2 mb-3">
                                    {{ Str::limit(strip_tags($announcement->content), 150) }} </p>
                                <!-- Attachments Preview -->
                                @if ($announcement->attachments->count() > 0)
                                    <div class="flex items-center space-x-2 mb-3">
                                        @foreach ($announcement->attachments->take(3) as $attachment)
                                            ...
                                            @endforeach @if ($announcement->attachments->count() > 3)
                                                <span
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3"> <a
                                                ...
                                                class="text-blue-600 hover:text-blue-800 text-sm font-medium"> Read More →
                                            </a>
                                            @if (auth()->user()->canManageAnnouncement($announcement))
                                                <a href="{{ route('announcements.edit', $announcement) }}"
                                                    class="text-gray-500 hover:text-gray-700 text-sm"> Edit </a>
                                                @endif
                                        </div> <!-- Time Ago --> <span class="text-xs text-gray-400">
                                            {{ $announcement->published_at->diffForHumans() }} </span>
                                    </div>
                            </div>
                        </div>
                </div> @empty <div class="p-12 text-center"> <svg class="w-16 h-16 mx-auto text-gray-300 mb-4"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if (auth()->user()->canCreateAnnouncements())
                            <a href="{{ route('announcements.create') }}"
                                ...
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 4v16m8-8H4" />
                                </svg> Create First Announcement </a>
                            @endif
                    </div> @endforelse
            </div> <!-- Pagination -->
            @if ($announcements->hasPages())
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50/50"> {{ $announcements->links() }} </div>
            @endif
        </div>
    </div> @push('scripts')
        <script>
