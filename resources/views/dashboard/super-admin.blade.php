...
<div class="flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-medium bg-gradient-to-r from-red-600 via-orange-600 to-amber-600 bg-clip-text text-transparent tracking-tight">System Administration</h1>
        <p class="text-gray-600 mt-1">{{ $userRole }} - Full System Access</p>
    </div>
        ...
        <div class="relative p-6">
            <div class="flex items-center">
                ...
                <div class="ml-5 flex-1">
                    <p class="text-sm font-medium text-gray-600 uppercase tracking-wider">👥 User Management</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $adminStats['totalUsers'] ?? 0 }}</p>
                    <div class="flex items-center mt-2">
                        <div class="w-2 h-2 bg-blue-400 rounded-full mr-2"></div>
                        <span class="text-xs text-gray-600">{{ $adminStats['activeUsers'] ?? 0 }} active</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="bg-gray-50 px-6 py-3 border-t border-gray-100">
            <a href="{{ route('admin.users.index') }}" class="flex items-center justify-between text-sm font-medium text-gray-700 hover:text-blue-600 group transition-colors">
    ...
    <div class="group relative bg-white overflow-hidden rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 border border-gray-100">
        <div class="relative p-6">
            <div class="flex items-center">
                ...
                <div class="ml-5 flex-1">
                    <p class="text-sm font-medium text-gray-600 uppercase tracking-wider">📄 Content</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ ($adminStats['totalAnnouncements'] ?? 0) + ($adminStats['totalDocuments'] ?? 0) }}</p>
                    <div class="flex items-center mt-2">
                        <div class="w-2 h-2 bg-green-400 rounded-full mr-2"></div>
                        <span class="text-xs text-gray-600">All content types</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="bg-gray-50 px-6 py-3 border-t border-gray-100">
            <a href="{{ route('admin.content.index') }}" class="flex items-center justify-between text-sm font-medium text-gray-700 hover:text-green-600 group transition-colors">
    ...
    <div class="group relative bg-white overflow-hidden rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 border border-gray-100">
        <div class="relative p-6">
            <div class="flex items-center">
                ...
                <div class="ml-5 flex-1">
                    <p class="text-sm font-medium text-gray-600 uppercase tracking-wider">⚙️ System</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">Settings</p>
                    <div class="flex items-center mt-2">
        ...
        <div class="bg-gray-50 px-6 py-3 border-t border-gray-100">
            <a href="{{ route('admin.settings.index') }}" class="flex items-center justify-between text-sm font-medium text-gray-700 hover:text-purple-600 group transition-colors">
        ...
        <div class="relative p-6">
            <div class="flex items-center">
                ...
                <div class="ml-5 flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1"><span id="vault-count">0</span></p>
                    <div class="flex items-center mt-2">
...
<div class="mb-8">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            <a href="{{ route('admin.users.create') }}" class="flex flex-col items-center p-4 rounded-lg border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 transition-colors group">
                ...
                <span class="text-sm font-medium text-gray-700 group-hover:text-indigo-700">Add User</span>
            </a>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-blue-600">{{ $adminStats['totalUsers'] ?? 0 }}</div>
                        <div class="text-sm text-gray-600">Total Users</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-green-600">{{ $adminStats['totalAnnouncements'] ?? 0 }}</div>
                        <div class="text-sm text-gray-600">Announcements</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-purple-600">{{ $adminStats['totalDocuments'] ?? 0 }}</div>
                        <div class="text-sm text-gray-600">Documents</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-yellow-600">{{ $adminStats['totalPolls'] ?? 0 }}</div>
                            ...
                            <div class="flex items-start space-x-4 p-4 rounded-lg border border-gray-100">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center mt-2 space-x-4">
                                        ...
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-{{ $announcement->priority === 'urgent' ? 'red' : ($announcement->priority === 'high' ? 'orange' : 'gray') }}-100 text-{{ $announcement->priority === 'urgent' ? 'red' : ($announcement->priority === 'high' ? 'orange' : 'gray') }}-800">{{ ucfirst($announcement->priority) }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
    ...
    <div class="space-y-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            <div class="w-2 h-2 bg-green-400 rounded-full mr-1"></div>Healthy
                        </span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-gray-600">Storage</span>
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            <div class="w-2 h-2 bg-green-400 rounded-full mr-1"></div>OK
                        </span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-gray-600">Cache</span>
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            <div class="w-2 h-2 bg-green-400 rounded-full mr-1"></div>Active
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="p-6 border-b border-gray-100">
                <h3 class="text-lg font-semibold text-gray-900">🔗 Admin Links</h3>
            </div>
            <div class="p-6">
                <div class="space-y-3">
                    <a href="{{ route('admin.reports.index') }}" class="flex items-center p-3 rounded-lg border border-gray-100 hover:border-gray-200 hover:bg-gray-50 transition-colors group">
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900 group-hover:text-blue-600">System Reports</h4>
                            <p class="text-sm text-gray-600 mt-1">View analytics and reports</p>
                        </div>
                        <svg class="w-4 h-4 text-gray-400 group-hover:text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </a>
                    <a href="{{ route('admin.backup.index') }}" class="flex items-center p-3 rounded-lg border border-gray-100 hover:border-gray-200 hover:bg-gray-50 transition-colors group">
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900 group-hover:text-blue-600">System Backup</h4>
                            <p class="text-sm text-gray-600 mt-1">Backup and restore data</p>
                        </div>
                        <svg class="w-4 h-4 text-gray-400 group-hover:text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </a>
                    <a href="{{ route('admin.logs.index') }}" class="flex items-center p-3 rounded-lg border border-gray-100 hover:border-gray-200 hover:bg-gray-50 transition-colors group">
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900 group-hover:text-blue-600">System Logs</h4>
                            <p class="text-sm text-gray-600 mt-1">Monitor system activity</p>
                        </div>
                        <svg class="w-4 h-4 text-gray-400 group-hover:text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
