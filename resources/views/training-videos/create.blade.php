...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-semibold text-gray-900">Add Training Video</h1>
                        <p class="text-lg text-gray-600 mt-1">Share knowledge with your team</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('training-videos.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="mb-6">
                ...
                <input type="url" id="video_url" name="video_url" value="{{ old('video_url') }}" required
                    placeholder="https://www.youtube.com/watch?v=... or https://vimeo.com/... or file path"
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Supports YouTube, Vimeo, or local video files. The video type will be automatically detected.
                </p>
            </div>
                    ...
                    <div>
                        ...
                        <select id="target_audience" name="target_audience"
                            ...
                            @foreach ($targetAudiences as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('target_audience') === $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="duration_minutes" name="duration_minutes"
                            value="{{ old('duration_minutes') }}" min="1" placeholder="e.g., 15"
                    ...
                    <div id="videoPreview" class="hidden">
                        ...
                        <div class="border border-gray-300 rounded-lg overflow-hidden">
                            <div id="previewContainer" class="aspect-video bg-gray-100 flex items-center justify-center">
    ...
    <script>
        document.getElementById('video_url').addEventListener('blur', function() {
            ...
            // Auto-detect video type and show preview
            if (url.includes('youtube.com') || url.includes('youtu.be')) {
                showYouTubePreview(url);
            } else if (url.includes('vimeo.com')) {
                showVimeoPreview(url);
        ...
        function showYouTubePreview(url) {
            ...
            if (videoId) {
                ...
                document.getElementById('previewContainer').innerHTML =
                    `<img src="${thumbnailUrl}" alt="YouTube Preview" class="w-full h-full object-cover">`;
        ...
        function showVimeoPreview(url) {
            const videoId = extractVimeoId(url);
            if (videoId) {
                // Vimeo API requires more complex handling, show generic preview
                document.getElementById('previewContainer').innerHTML =
        ...
        function showVideoFilePreview(url) {
            document.getElementById('previewContainer').innerHTML =
        ...
        function fetchYouTubeTitle(videoId) {
            // This would require YouTube API key, so we'll skip auto-title for now
            // You can implement this if you have a YouTube API key
        }

        // Auto-fill title based on URL if title is empty
        document.getElementById('video_url').addEventListener('change', function() {
            const titleField = document.getElementById('title');
            if (!titleField.value.trim()) {
                const url = this.value.trim();
                if (url.includes('youtube.com') || url.includes('youtu.be')) {
                    titleField.placeholder = 'YouTube video title will be detected...';
                } else if (url.includes('vimeo.com')) {
                    titleField.placeholder = 'Vimeo video title will be detected...';
                }
            }
        });
    </script>
