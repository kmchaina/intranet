...
@section('content')
            ...
            <div class="bg-white rounded-lg shadow-md border border-gray-200 p-6">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="content" id="content" rows="12" required placeholder="Write your news content here..."
                        class="block w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-teal-500 focus:border-teal-500 sm:text-sm @error('content') border-red-300 @enderror">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select name="location" id="location" required
                            ...
                            <option value="headquarters" {{ old('location') == 'headquarters' ? 'selected' : '' }}>
                                Headquarters</option>
                            <option value="mwanza" {{ old('location') == 'mwanza' ? 'selected' : '' }}>Mwanza</option>
                            <option value="mbeya" {{ old('location') == 'mbeya' ? 'selected' : '' }}>Mbeya</option>
                            <option value="tanga" {{ old('location') == 'tanga' ? 'selected' : '' }}>Tanga</option>
                            <option value="tabora" {{ old('location') == 'tabora' ? 'selected' : '' }}>Tabora</option>
                            <option value="dodoma" {{ old('location') == 'dodoma' ? 'selected' : '' }}>Dodoma</option>
                            <option value="amani" {{ old('location') == 'amani' ? 'selected' : '' }}>Amani</option>
                            <option value="mpwapwa" {{ old('location') == 'mpwapwa' ? 'selected' : '' }}>Mpwapwa</option>
                        </select>
                        @error('location')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
    ...
    @push('scripts')
        <script>
            ...
            // Auto-set location based on user's assignment (if available)
            document.addEventListener('DOMContentLoaded', function() {
                const locationSelect = document.getElementById('location');

                // You can get user's location from backend and set it here
                // For now, we'll leave it as manual selection
