    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-semibold text-gray-900">Edit Task</h1>
                    <p class="text-lg text-gray-600 mt-1">Update your task details</p>
                </div>
        ...
        <div class="p-8">
            <form method="POST" action="{{ route('todos.update', $todo) }}" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $todo->title) }}"
                        ...
                        placeholder="e.g., Complete monthly report, Call the client, Review budget" required>
                    @error('title')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg text-lg focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Add any additional details about this task...">{{ old('description', $todo->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="space-y-3">
                        <label
                            ...
                            <span class="ml-3 text-lg">🔴 High Priority (urgent, must be done soon)</span>
                        </label>
                        ...
                        <label
                            ...
                            <span class="ml-3 text-lg">🟡 Medium Priority (important, but not urgent)</span>
                        </label>
                        ...
                        <label
                            ...
                            <span class="ml-3 text-lg">🟢 Low Priority (can wait, nice to have done)</span>
                        </label>
                    </div>
                    @error('priority')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <!-- Due Date -->
                <div class="mb-6">
                    <label for="due_date" class="block text-lg font-medium text-gray-700 mb-2">
                        When does this need to be done? (optional)
                    </label>
                    <input type="date" name="due_date" id="due_date"
                        value="{{ old('due_date', $todo->due_date ? \Carbon\Carbon::parse($todo->due_date)->format('Y-m-d') : '') }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg text-lg focus:ring-blue-500 focus:border-blue-500">
                    <p class="mt-1 text-sm text-gray-600">Leave blank if there's no specific deadline</p>
                    @error('due_date')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="space-y-3">
                        <label
                            ...
                            <input type="radio" name="completed" value="0"
                                {{ old('completed', $todo->completed ? '1' : '0') === '0' ? 'checked' : '' }}
                                class="w-5 h-5 text-yellow-600 focus:ring-yellow-500 border-gray-300">
                            <span class="ml-3 text-lg">❌ Not yet completed</span>
                        </label>

                        <label
                            class="flex items-center p-4 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer">
                            <input type="radio" name="completed" value="1"
                                {{ old('completed', $todo->completed ? '1' : '0') === '1' ? 'checked' : '' }}
                                class="w-5 h-5 text-green-600 focus:ring-green-500 border-gray-300">
                            <span class="ml-3 text-lg">✅ Task is completed</span>
                        </label>
                    </div>
                    @error('completed')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
