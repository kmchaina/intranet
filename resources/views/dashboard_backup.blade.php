...
[
'title' => 'NIMR Centres',
'value' => App\Models\Centre::where('is_active', true)->count(),
...
'value' => App\Models\Station::where('is_active', true)->count(),
...
@foreach ($stats as $stat)
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-card card-hover">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-green-600 dark:text-green-400 mt-1">{{ $stat['change'] }}</p>
            </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <!-- Recent Announcements -->
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-card">
        ...
        @php
            $recentAnnouncements = App\Models\Announcement::forUser(auth()->user())
                ->latest()
                ->take(3)
                ->get();
        @endphp
        ...
        @else
            <p class="text-gray-500 dark:text-gray-400 text-center py-8">No announcements yet</p>
        @endif
    </div>
            ...
            @php
                $quickActions = [
                    [
                        ...
                        'color' => 'bg-blue-500',
                        'can' => auth()->user()->canCreateAnnouncements(),
                    ],
                    [
                        'title' => 'View Team',
                        'description' => 'Browse organizational structure',
                        'icon' => 'fas fa-users',
                        'route' => '#',
                        'color' => 'bg-green-500',
                        'can' => true,
            ...
            @foreach ($quickActions as $action)
                @if ($action['can'])
                    <a href="{{ $action['route'] !== '#' ? route($action['route']) : '#' }}"
                        class="block p-4 rounded-lg border-2 border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-500 transition-colors card-hover">
                        <div
                            class="{{ $action['color'] }} w-10 h-10 rounded-lg flex items-center justify-center mb-3">
                            <i class="{{ $action['icon'] }} text-white"></i>
                        </div>
                        <h4 class="font-medium text-gray-900 dark:text-white text-sm">{{ $action['title'] }}</h4>
...
@if (auth()->user()->isSuperAdmin() || auth()->user()->isHqAdmin())
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-card">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Organizational Overview</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <!-- Role Distribution -->
            <div class="text-center">
                <h4 class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">Role Distribution</h4>
                @php
                    $roleCounts = [
                        'super_admin' => App\Models\User::where('role', 'super_admin')->count(),
                        'hq_admin' => App\Models\User::where('role', 'hq_admin')->count(),
                        'centre_admin' => App\Models\User::where('role', 'centre_admin')->count(),
                        'station_admin' => App\Models\User::where('role', 'station_admin')->count(),
                ...
                <div class="space-y-2">
                    @foreach ($roleCounts as $role => $count)
                        <div class="flex items-center justify-between text-sm">
                            <span
                                class="text-gray-600 dark:text-gray-400 capitalize">{{ str_replace('_', ' ', $role) }}</span>
            ...
            <!-- Centre Status -->
            <div class="text-center">
                <h4 class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">Research Centres</h4>
                <div class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                    {{ App\Models\Centre::where('is_active', true)->count() }}
                </div>
                <p class="text-sm text-green-600 dark:text-green-400">Active centres</p>
            </div>

            <!-- Station Status -->
            <div class="text-center">
                <h4 class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">Research Stations</h4>
                <div class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                    {{ App\Models\Station::where('is_active', true)->count() }}
                </div>
                <p class="text-sm text-green-600 dark:text-green-400">Active stations</p>
            </div>
        </div>
    </div>
@endif
