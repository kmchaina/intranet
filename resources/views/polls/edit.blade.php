            ...
            <form method="POST" action="{{ route('polls.update', $poll) }}">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        placeholder="Optional description of the poll">{{ old('description', $poll->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @if (!$poll->responses()->exists())
                    ...
                    <div id="options-section" class="mb-6"
                        ...
                        <div id="options-container">
                            ...
                            @else
                                ...
                                <div class="flex items-center mb-2 option-row">
                                    <input type="text" name="options[]" value="{{ old('options.1') }}"
                                        class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        ...
                        <button type="button" id="add-option" class="mt-2 text-blue-600 hover:text-blue-800">
                            + Add Option
                        </button>
                        @error('options')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="mb-6">
                        <label class="flex items-center">
                            ...
                            <span class="ml-2 text-sm text-gray-700">Anonymous voting (responses won't be linked to
                                users)</span>
                        </label>
                        @error('anonymous')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                @else
                    ...
                    <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                        <h4 class="font-medium text-gray-700 mb-2">Current Poll Settings</h4>
                        <div class="text-sm text-gray-600 space-y-1">
                            <p><strong>Type:</strong> {{ $poll->getTypeLabel() }}</p>
                            @if ($poll->options)
                                <p><strong>Options:</strong> {{ implode(', ', $poll->options) }}</p>
                            @endif
                            @if ($poll->max_rating)
                                <p><strong>Rating Scale:</strong> 1 to {{ $poll->max_rating }}</p>
                            @endif
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="show_results" value="1"
                                ...
                                class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            <span class="ml-2 text-sm text-gray-700">Show results to voters</span>
                        </label>
                    </div>
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="allow_comments" value="1"
                                ...
                                class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            <span class="ml-2 text-sm text-gray-700">Allow comments with votes</span>
                        </label>
                    </div>
                </div>
                ...
                <div id="department-section" class="mb-6"
                    ...
                    <div class="space-y-2 max-h-40 overflow-y-auto border border-gray-300 rounded-md p-3">
                        @foreach ($departments as $department)
                            <label class="flex items-center">
                                <input type="checkbox" name="visible_departments[]" value="{{ $department }}"
                                    {{ in_array($department, old('visible_departments', $poll->visible_to ?? [])) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <span class="ml-2 text-sm">{{ $department }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div id="user-section" class="mb-6"
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto border border-gray-300 rounded-md p-3">
                        @foreach ($users->groupBy('department') as $department => $departmentUsers)
                            <div class="mb-3">
                                <h4 class="font-medium text-gray-700 mb-1">{{ $department ?: 'No Department' }}</h4>
                ...
                <div class="mb-6">
                    ...
                    <select name="status" id="status"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        required>
                        <option value="draft" {{ old('status', $poll->status) === 'draft' ? 'selected' : '' }}>Draft
                        </option>
                        <option value="active" {{ old('status', $poll->status) === 'active' ? 'selected' : '' }}>Active
                        </option>
                        <option value="closed" {{ old('status', $poll->status) === 'closed' ? 'selected' : '' }}>Closed
                        </option>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function toggleSections() {
                    ...
                    optionsSection.style.display = ['single_choice', 'multiple_choice'].includes(type) ? 'block' :
                        'none';
                ...
                function toggleVisibilitySections() {
                    ...
                    departmentSection.style.display = visibility === 'department' ? 'block' : 'none';
                    userSection.style.display = visibility === 'custom' ? 'block' : 'none';
                }
