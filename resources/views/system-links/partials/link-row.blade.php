...
<div
    class="group bg-white border border-gray-200 rounded-lg hover:shadow-md transition-all duration-200 hover:-translate-y-0.5">
    <div class="p-4">
        <div class="flex items-start justify-between">
            ...
            <div class="flex items-start space-x-3 flex-1 min-w-0">
                @if ($link->icon)
                    <div
                        class="flex-shrink-0 w-10 h-10 {{ $colorClasses[0] }} rounded-lg flex items-center justify-center">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center space-x-2 mb-1">
                        <a href="#"
                            ...
                            class="text-sm font-semibold text-gray-900 hover:text-green-600 transition-colors truncate group-hover:text-green-600">
                            {{ $link->title }}
                        </a>
                        @if ($link->is_featured)
                            <span
                        ...
                        @if ($link->requires_vpn)
                            <span
                                class="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-700">
                                🔒 VPN
                            </span>
                        @endif
                    </div>
                    ...
                    @if ($link->description)
                        <p class="text-sm text-gray-600 line-clamp-2 mb-2">{{ $link->description }}</p>
                    @endif

                    <div class="flex items-center space-x-4 text-xs text-gray-500">
                        <span class="inline-flex items-center">
                            <span class="w-2 h-2 {{ $colorClasses[0] }} rounded-full mr-1.5"></span>
                        ...
                        @if ($link->click_count > 0)
                            <span class="inline-flex items-center">
                                ...
                                {{ number_format($link->click_count) }} clicks
                            </span>
                        @endif
            ...
            <div class="relative flex-shrink-0 ml-4">
                <button onclick="toggleDropdown('dropdown-{{ $link->id }}')"
                ...
                <div id="dropdown-{{ $link->id }}"
                    ...
                    <div class="py-1">
                        ...
                        @endcan
                        @can('delete', $link)
                            <form action="{{ route('system-links.destroy', $link) }}" method="POST" class="block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
