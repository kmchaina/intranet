...
@section('title', 'Manage Centre Staff')
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-1">Staff members and station admins in your centre</p>
            </div>
                ...
                <div>
                    ...
                    <select name="station_id"
                        ...
                        @foreach ($stations as $station)
                            <option value="{{ $station->id }}" @selected(request('station_id') == $station->id)>
                                {{ $station->name }}
                            </option>
                        @endforeach
                    </select>
                        ...
                        @forelse ($users as $user)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm">
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $user->role === 'station_admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700' }}">
                                            {{ $user->role === 'station_admin' ? 'Station Admin' : 'Staff' }}
                                        </span>
                                        @if ($user->station)
                                            <p class="text-xs text-gray-500 mt-1">{{ $user->station->name }}</p>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form id="deleteForm{{ $user->id }}"
                                            action="{{ route('admin.centre.users.destroy', $user) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                                onclick="showConfirmModal({
                                                    ...
                                                    message: 'This will permanently remove this user from the system.',
