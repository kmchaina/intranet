...
@section('content')
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('admin.users.store') }}">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="date" id="birth_date" name="birth_date" value="{{ old('birth_date') }}"
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('birth_date') border-red-300 @enderror">
                                    @error('birth_date')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <!-- Hire Date -->
                                <div class="mb-4">
                                    <label for="hire_date" class="block text-sm font-medium text-gray-700">Hire
                                        Date</label>
                        ...
                        <div class="space-y-6">
                            <div>
                                <h3 class="text-lg font-medium text-gray-900 mb-4">Role & Organizational Assignment
                                </h3>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select id="role" name="role" required
                                        ...
                                        <option value="centre_admin" {{ old('role') == 'centre_admin' ? 'selected' : '' }}>
                                            Centre Admin</option>
                                        <option value="hq_admin" {{ old('role') == 'hq_admin' ? 'selected' : '' }}>HQ Admin
                                        </option>
                                    ...
                                    <div
                                        ...
                                        <p><strong>Super Admin:</strong> Full access everywhere; ignore centre/station
                                            fields.</p>
                                        <p><strong>HQ Admin:</strong> Oversees all centres/stations but shouldn’t be tied to
                                            one.</p>
                                        <p><strong>Centre Admin:</strong> Must select a centre; station optional.</p>
                                        <p><strong>Station Admin:</strong> Must select both centre and station.</p>
                                        <p><strong>Staff:</strong> Optional centre/station based on assignment.</p>
                                    </div>
                                </div>

                                <!-- Centre Selection -->
                                <div class="mb-4" id="centre-selection">
                                ...
                                <!-- Station Selection -->
                                <div class="mb-4" id="station-selection">
                                    ...
                                    <select id="station_id" name="station_id"
                                        ...
                                        @foreach ($stations as $station)
                                            <option value="{{ $station->id }}"
                                                data-centre-id="{{ $station->centre_id }}"
                                                {{ old('station_id') == $station->id ? 'selected' : '' }}>
                                                {{ $station->name }} ({{ $station->code }}) -
                                                {{ $station->centre?->name ?? 'No Centre' }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('station_id')
                                ...
                                <div class="mb-4" id="department-selection">
                                    ...
                                    <select id="department_id" name="department_id"
                                        ...
                                        <!-- Departments will be loaded dynamically based on centre selection -->
                                    </select>
                                    @error('department_id')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const centreSelection = document.getElementById('centre-selection');
            ...
            function updateOrganizationalVisibility() {
                const role = roleSelect.value;
                centreSelection.style.display = 'none';
                stationSelection.style.display = 'none';
                departmentSelection.style.display = 'none';

                if (role === 'centre_admin' || role === 'station_admin' || role === 'staff') {
                    centreSelection.style.display = 'block';
                    departmentSelection.style.display = 'block';
                }

                if (role === 'station_admin' || (role === 'staff' && centreSelect.value)) {
            ...
            function updateStationOptions() {
                ...
                const stationOptions = stationSelect.querySelectorAll('option[data-centre-id]');

                stationOptions.forEach(option => {
                    if (selectedCentreId === '' || option.getAttribute('data-centre-id') ===
                        selectedCentreId) {
                ...
                if (selectedCentreId && stationSelect.value) {
                    const selectedStation = stationSelect.querySelector(`option[value="${stationSelect.value}"]`);
                    if (selectedStation && selectedStation.getAttribute('data-centre-id') !== selectedCentreId) {
                        stationSelect.value = '';
                    }
                }
            }

            roleSelect.addEventListener('change', updateOrganizationalVisibility);
            centreSelect.addEventListener('change', updateStationOptions);
            updateOrganizationalVisibility();
        });
    </script>
@endsection
